...
RELEVANCE_CONTEXT = {
    ...
    "encreative": "이엔크리에이티브 또는 그 브랜드(국민학교 떡볶이·국떡·밀키트)가 '주체'인 소식(신제품·실적·입점·납품·수출·투자 등). 타 회사(항공·카드·유통사 등)가 주체이고 밀키트·떡볶이가 스치듯 언급된 기부·행사 기사는 관련 없음",
...
def relevance_filter(articles: list, ctx_key: str, batch: int = 20) -> list:
    ...
    for i in range(0, len(articles), batch):
        ...
        system = (
            "너는 엄격한 뉴스 관련성 판별기다. 핵심 기준: 기사의 '주체(주인공)'가 주제의 회사/대상인가? "
            "주체가 다른 회사이고 주제 키워드가 스치듯 언급될 뿐이면 반드시 관련 없음으로 분류한다. "
            "예1: 통신 주제인데 기사 주체가 언론사(AP통신·교도통신)·정치인·연예인·위성통신 기기면 관련 없음. "
            "예2: '대한항공이 밀키트를 기부'처럼 주체가 항공사이고 밀키트는 기부품으로만 나오면 관련 없음. "
            "예3: 반대로 '국떡이 대한항공 기내식에 납품'처럼 주체가 해당 브랜드면 관련 있음. "
            "애매하면 주체가 누구인지로 판단하라. "
