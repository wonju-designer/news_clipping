...
CATEGORIES = [
    {
        ...
        "keywords": [
            ...
            "중고폰", "SKT", "KT", "LG U+", "LG유플러스", "엘지유플러스",
            "에스케이텔레콤", "케이티", "휴대전화", "갤럭시", "아이폰",
            "명의도용", "휴대폰", "개통",
        ],
        # 노출/선별 우선순위: 아래 표현이 든 기사를 상단에 배치
        "priority_terms": ["이동통신", "MNO", "MVNO", "알뜰폰"],
    ...
    },
    {
        ...
        # 노출/선별 우선순위: 알뜰폰·알뜰요금제·MVNO가 든 기사를 상단에 배치
        "priority_terms": ["알뜰폰", "알뜰요금제", "MVNO"],
        # 게임·애니·연예 오탐 + LG유플러스(MNO) 제외 (이 카테고리 한정)
        "exclude": [
    ...
    },
    {
        ...
        # 우선순위: 자사(머큐리) 뱃지 / 산업(광통신) 뱃지 각각
        "priority_own": ["광통신", "(주)머큐리", "머큐리 주식회사"],
        "priority_industry": ["광통신", "광케이블"],
        # 머큐리 직접 언급 → '자사' 뱃지
        "keywords": ["머큐리", "(주)머큐리", "100590"],
        # 머큐리 사업영역(광통신·네트워크) → '산업' 뱃지, 자회사 소식으로 정리
        "industry_keywords": ["광통신", "광케이블", "광전복합케이블", "네트워크 사업"],
        # 동명이인/타사/영화/승마 등 오탐 제외 (이 카테고리에만 적용)
        "exclude": [
            "프레디 머큐리", "퀸", "머큐리 영화", "수성",
